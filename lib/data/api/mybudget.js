...
export function extractBudgets (budgets, cb) {
	...
	for(var i = 0; i < budgets.length; i++) {
    	...
    	let categoryName = categoryBudget.name
    	let categoryAmount = new Number(parseFloat(categoryBudget.amount))

    	// get a category by name
    	CategoryApi.getCategoryByName(categoryName, 
    		function err() {
	    		console.log('Error getting category: ', categoryName, err)
			    return handleError(res)
    		},
    		function success(data) {
			    //log(`Retrieving category: %j`, data)
